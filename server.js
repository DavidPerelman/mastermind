...
io.on('connection', (socket) => {
  ...
  socket.on('join-room', (code) => {
    ...
    if (!room) { socket.emit('join-error', 'חדר לא נמצא'); return; }
    if (room.players.length >= 2) { socket.emit('join-error', 'החדר מלא'); return; }
  ...
  socket.on('player-ready', ({ secret } = {}) => {
    ...
    if (room.readyCount === 2) {
      ...
      io.to(room.code).emit('game-start', { settings: room.settings, firstTurn: 0 });
    }
  });
  ...
  socket.on('player-won', ({ guessCount }) => {
    ...
    io.to(room.code).emit('player-won', { playerIndex: idx, guessCount: room.guessCount[idx] });
    checkGameOver(room);
  });
  ...
  socket.on('player-lost', () => {
    ...
    const idx = playerIndex(room, socket.id);
    room.finished[idx] = true;
    io.to(room.code).emit('player-lost', { playerIndex: idx, guessCount: room.guessCount[idx] });
    checkGameOver(room);
  });
